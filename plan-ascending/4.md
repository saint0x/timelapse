...
- [ ] Create `crates/seer-cli/Cargo.toml`
...
- [ ] Implement `cmd/init.rs`
  - [ ] `pub async fn run() -> Result<()>`
    ```rust
    // 1. Check if already initialized (.snap/ exists)
...
- [ ] Create `.snap/config.toml` with defaults
  ```toml
  [retention]
  dense_count = 2000
  dense_window = "24h"
...
- [ ] Implement `cmd/gc.rs`
  - [ ] `pub async fn run(dry_run: bool) -> Result<()>`
    ```rust
    // 1. Acquire GC lock (.snap/locks/gc.lock)
    // 2. Run GC algorithm (from seer-journal)
...
- [ ] Implement daemon lifecycle
  - [ ] `start() -> Result<()>`
    ```rust
    // 1. Check if daemon already running (check .snap/locks/daemon.lock)
...
- [ ] Unix socket location: `.snap/state/daemon.sock`
...
- [ ] CLI uses IPC to query daemon
  - [ ] `snap status` → `GetStatus` request
  - [ ] `snap log` → query journal directly (no IPC needed)
  - [ ] `snap stop` → `Shutdown` request
...
- [ ] Implement background daemon spawn
  ...
  - [ ] Redirect stdout/stderr to `.snap/logs/daemon.log`
...
- [ ] **Decision**: Manual start for MVP
  - [ ] User runs `snap start` explicitly
...
- [ ] Daemon logs to `.snap/logs/daemon.log`
...
- [ ] Metrics collection
  - [ ] Write to `.snap/state/metrics.json`
  - [ ] Track: checkpoints/hour, bytes written, errors
  - [ ] Use for `snap status` display
...
- [ ] End-to-end test
  - [ ] `snap init` → `snap start` → make changes → `snap log` → `snap restore`
...
- [ ] Error messages
  ...
  - [ ] Suggestions: "Did you mean `snap init`?"
