...
- [ ] Create `crates/seer-jj/Cargo.toml`
...
## Command: `snap publish`

- [ ] Implement `cmd/publish.rs` in seer-cli
...
- [ ] Support publish targets
  - [ ] Single checkpoint: `snap publish HEAD`
  - [ ] Range: `snap publish HEAD~10..HEAD` (create 10 JJ commits)
  - [ ] Named pin: `snap publish last-good`

- [ ] Bookmark naming
  - [ ] Default: `snap/<checkpoint-id>`
  - [ ] Custom: `snap publish HEAD -b my-feature`
...
- [ ] Implement `cmd/push.rs` in seer-cli
...
- [ ] Implement `cmd/pull.rs` in seer-cli
...
- [ ] **MVP decision**: Manual import
  ...
  - [ ] User runs `snap pull --import` to create checkpoint from JJ head
...
- [ ] Store bidirectional mapping
  - [ ] After `snap publish`: record `checkpoint_id -> jj_commit_id`
  - [ ] After `snap pull --import`: record `checkpoint_id -> jj_commit_id`
  - [ ] Use for `snap push` to determine what to push
...
- [ ] Integration tests with JJ
  ...
  - [ ] Test: `snap publish` → `jj log` (verify commit created)
  - [ ] Test: `snap push` → verify Git remote updated
  - [ ] Test: `snap pull --import` → verify checkpoint created
...
- [ ] Conflict resolution UI
  - [ ] When `snap pull` conflicts with local changes
  - [ ] Show diff, let user choose resolution

- [ ] Bookmark management
  - [ ] `snap jj-bookmarks` to list all snap/* bookmarks
  - [ ] `snap jj-cleanup` to remove old bookmarks
