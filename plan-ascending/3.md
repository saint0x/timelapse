...
- [ ] Create `crates/seer-journal/Cargo.toml`
  - [ ] Dependencies: sled, serde, bincode, ulid, chrono, parking_lot
  - [ ] Add seer-core as workspace dependency
...
- [ ] Initialize journal database
  ...
  - [ ] Open sled DB at `.snap/journal/checkpoints.db`
...
- [ ] Implement `pathmap.rs`: In-memory working state
  ...
  // Entry from seer-core::tree
  ```

- [ ] Implement persistence
  - [ ] `save(&self, path: &Path) -> Result<()>`
    - [ ] Serialize to `.snap/state/pathmap.bin`
...
- [ ] Implement pin management
  - [ ] `pin(&self, id: CheckpointId, name: String) -> Result<()>`
    - [ ] Write to `.snap/refs/pins/<name>`
    - [ ] File contains checkpoint ID
  - [ ] `unpin(&self, name: &str) -> Result<()>`
    - [ ] Remove `.snap/refs/pins/<name>`
  - [ ] `list_pins(&self) -> Result<Vec<Pin>>`
    - [ ] Read all files in `.snap/refs/pins/`
...
- [ ] Implement background GC
  - [ ] Acquire `.snap/locks/gc.lock` (prevent concurrent GC)
...
- [ ] Implement crash recovery
  ...
  - [ ] Delete incomplete checkpoint writes in `.snap/tmp/`
